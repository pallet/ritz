(ns ritz.repl-utils.indent
  (:require
   [ritz.repl-utils.helpers :as helpers]
   [ritz.repl-utils.utils :as utils]))

(defn indentation-hash
  "Provides a hash of the namespace data.
   This allows a heuristic so as to avoid scanning all symbols from all
   namespaces. Instead, we only check whether the set of namespaces in the cache
   match the set of currently defined namespaces."
  []
  (hash (all-ns)))
...
(defn- var-indent-representation
  "Returns an indentation representation [name position] for
  ...
  (when-let [body-position (find-var-body-position var)]
    (when (or (= body-position 'defun) (not (neg? body-position)))
      [(name (:name (meta var))) body-position])))
...
(defn- update-indentation-delta
  ...
  [ns all-ns cache load-all-ns?]
  (mapcat
   (partial get-cache-updates-in-namespace cache)
   (if load-all-ns? all-ns [ns])))
...
(defn indentation-update
  ...
  [indent-context ns]
  (let [all-ns (all-ns)
        indent-hash (hash all-ns)]
    {:cache-hash indent-hash
     :updates (update-indentation-delta
               ns all-ns (:indent-cache indent-context)
               (not= indent-hash (:cache-hash indent-context)))}))
