(ns ritz.jpda.debug
  ...
  (:use
   ...
   [ritz.exception-filters :only [exception-filters exception-filter-add!]]
   [ritz.inspect :only [reset-inspector value-as-string]]
...
(defn exception-message
  ...
  (or
   ...
   (let [msg (jdi-clj/exception-message
     ...
     (break/break-exception-message!
      connection thread-id exception-event msg))))
...
(defn connection-for-event [event]
  (or (ritz.swank.connections/connection-for-event event)
      (ritz.nrepl.connections/connection-for-event event)))

(defn all-connections []
  (or (ritz.swank.connections/all-connections)
