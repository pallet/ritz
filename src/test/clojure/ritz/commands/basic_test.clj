...
(deftest connection-info-test
  (test-utils/eval-for-emacs-test
   ...
   #"\(:return \(:ok \(:pid \"\d+\" :style :spawn :lisp-implementation \(:type \"Clojure\" :name \"clojure\" :version \"1.[23].[0-1]([a-zA-Z-]+)?\"\) :package \(:name \"user\" :prompt \"user\"\) :version \"20101113\"\)\) 1\)"))
...
(deftest apropos-list-for-emacs-test
  ...
  (test-utils/eval-for-emacs-test
   ...
   (str "(:return (:ok ((:designator \"clojure.core/shutdown-agents\""
...
(deftest find-definitions-for-emacs-test
  ...
  (test-utils/eval-for-emacs-test
   ...
   #"\(:return \(:ok \(\(\"\(defn run-tests\)\" \(:location \(:zip \"[^\"]+\" \"clojure/test.clj\"\) \(:line \d+\) nil\)\)\)\) 1\)"))
